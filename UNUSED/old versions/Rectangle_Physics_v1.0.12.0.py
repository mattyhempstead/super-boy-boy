...
#   V1.0.12.0
#   
#   + Added text upon level completed
#   + Added list of records for levels
#
...
def centreX(label,rect):
    rectCentreX = rect[0] + (rect[2]/2) # find the centre of the rect to draw in
    textPos = label.get_rect()  # get rectangle the text fits into
    textPos[0] = rectCentreX - (textPos[2]/2)   # set the middle of the text to the centre of the rectangle
...
desert_background_img = createImage("cave_background.png",1080,720)
...
level_completed_pos = [centreX(level_completed_txt,[80,80,920,560]) ,130]
...
level[1].append([   480,        360,        240,        60,     GREEN,          True,       None    ])      # Middle Platform 1
level[1].append([   480,        300,        300,        60,     GREEN,          True,       None    ])      # Middle Platform 2
level[1].append([   480,        240,        360,        60,     GREEN,          True,       None    ])      # Middle Platform 3
...
while not done:
    ...
    if scene == "main_menu":
                ...
                if(boxes[i][5]):    # if boxes collision is true
                    if(rectToRectCollision(player,boxes[i])):   # if player colliding with box
                        ...
                        if boxes[i][6] == "finish":
                            ...
                            lvl_finish = True
                            lvl_timer_on = False
                            submitLevelTime(int(lvl_timer),int(scene[6:len(scene)]))
                        elif boxes[i][6] == "kill":
                            print("Player Died")
                            resetPlayer()
        ...
        if lvl_finish:
            ...
            timer_txt = font0.render("Time taken: " + str(lvl_timer/1000) + " seconds", 1, AQUA)
            timer_pos = [centreX(timer_txt,[80,80,920,560]) ,230]
            screen.blit(timer_txt, timer_pos)
            record_txt = font0.render("Level record: " + str(records[int(scene[6:len(scene)])]/1000) + " seconds", 1, AQUA)
            record_pos = [centreX(timer_txt,[80,80,920,560]) ,280]
            screen.blit(record_txt, record_pos)
